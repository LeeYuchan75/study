...
- 위에서 쪼개는 것에 핵심을 두어 계산했더니, 그룹별 오차들의 전체 평균을 최소화하려면, 그냥 **각 그룹의 오차를 개별적으로 최소화**하면 된다는 결론에 도달한다 (증명 생략)
...
- **위 공식 흐름을 이용한 예시** : 학생이 공부한 시간(X)을 알 때, 시험 점수(Y)를 가장 정확하게 예측하는 함수 f(x)를 찾아라 
    ...
    - ![System Resources](../../images/Artificial%20Neural%20Network%20images/EPE예시문제표.png)
    ...
    - 여기서 아래 공식을 사용해보자
        ...
        - 위 공식을 사용하기 위해선 f(x)가 필요한데 이것은 이전에 나온 것 처럼 아래 공식을 이요한다
