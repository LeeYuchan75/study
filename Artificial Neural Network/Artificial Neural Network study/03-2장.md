...
- 로지스틱 회귀 모델은 K개의 클래스에 대한 사후 확률(posterior probabilities)을 x의 선형 함수로 모델링하려는 데서 출발한다
    ...
    - 즉,  **softmax의 목표**는 K개 클래스의 선형점수를 exp로 변환해 **모든 확률이 [0,1] 이고 합이 1이 되도록 보장**하기 위함 
...
- 다항 로지스틱 회귀에서 소프트맥스와 동등한 공식으로 기준 클래스를 잡아서 표현할 수도 있다
    ...
    - 이렇게 구해서 나온 파라미터 베타를 세타 집합으로 표현하면 아래와 같다
 
    - ![System Resources](../../images/Artificial%20Neural%20Network%20images/03-2장다항로지스틱회귀베타집합.png)
 
    - 즉 입력 X = x가 주어졌을 때, 클래스 G = k일 확률은 아래와 같이 단순히 데이터 
x만으로 정해지는 게 아니라, 학습된 파라미터 θ 도 같이 판단을 해서 값을 구한다

    - ![System Resources](../../images/Artificial%20Neural%20Network%20images/03-2장다항로지스틱회귀세타를이용한확률표현.png)
