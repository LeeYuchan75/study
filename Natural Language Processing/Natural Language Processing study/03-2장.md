...
for doc in documents:
    # 한 문서를 대표할 벡터(값을 담는 공간)를 초기화합니다.
    vec = copy.copy(zero_vector)

    # 문서를 소문자로 바꾸고 단어(토큰) 단위로 쪼갭니다.
    tokens = tokenizer.tokenize(doc.lower())
    # 문서에 있는 각 단어의 개수를 셉니다.
    token_counts = Counter(tokens)
    
    # (단어, 단어 개수) 쌍으로 반복문을 실행합니다.
    for key, value in token_counts.items():
        # 이 단어가 몇 개 문서에 나왔는지 세는 변수를 초기화합니다.
        docs_containing_key = 0
        # 전체 문서를 다시 확인하며
        for _doc in documents:
            # 문서 안에 해당 단어가 있으면 카운트를 1 올립니다.
            if key in _doc:
        ...
        # 최종 TF-IDF 점수를 계산하여 벡터에 단어와 함께 저장합니다.
        vec[key] = tf * idf
    # 완성된 한 문서의 TF-IDF 벡터를 전체 리스트에 추가합니다.
    document_tfidf_vectors.append(vec)


# 질문 문장을 정의합니다.
query = "How long does it take to get to the store?"
# 질문 문장을 대표할 벡터를 초기화합니다.
query_vec = copy.copy(zero_vector)

# 질문을 소문자로 바꾸고 단어 단위로 쪼갭니다.
tokens = tokenizer.tokenize(query.lower())
# 질문에 있는 각 단어의 개수를 셉니다.
token_counts = Counter(tokens)

# 질문에 있는 (단어, 단어 개수) 쌍으로 반복문을 실행합니다.
for key, value in token_counts.items():
    # 이 단어가 몇 개 문서에 나왔는지 세는 변수를 초기화합니다.
    docs_containing_key = 0
    # 전체 문서들을 확인하며
    ...
    # 만약 단어가 어떤 문서에도 없었다면, IDF를 계산할 수 없으므로 건너뜁니다.
    if docs_containing_key == 0:
        continue

    # 질문 문장에서 해당 단어의 TF를 계산합니다.
    tf = value / len(tokens)
    # IDF를 계산합니다.
    idf = len(documents) / docs_containing_key
    # 최종 TF-IDF 점수를 계산하여 질문 벡터에 저장합니다.
    query_vec[key] = tf * idf

# 질문 벡터와 첫 번째 문서 벡터의 코사인 유사도를 계산하고 출력합니다.
print(cosine_sim(query_vec, document_tfidf_vectors[0]))
# 결과: 0.5235048549676834
# 질문 벡터와 두 번째 문서 벡터의 코사인 유사도를 계산하고 출력합니다.
print(cosine_sim(query_vec, document_tfidf_vectors[1]))
# 결과: 0.0
# 질문 벡터와 세 번째 문서 벡터의 코사인 유사도를 계산하고 출력합니다.
