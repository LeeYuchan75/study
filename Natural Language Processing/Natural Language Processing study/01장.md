...
- **1. 규칙 기반 접근**
    ...
    - 패턴 매칭을 기반으로 하여 자연어를 처리하는 방식

    - 정해진 규칙(패턴)에 맞는 글자를 찾아내서 문장을 처리하는 방법이다.
...
- **2. 통계 기반 접근**
    ...
    - 미리 확보한 데이터들을 바탕으로 조건부 확률을 활용하여 자연어를 해석 혹은 생성

    - 규칙 기반보다는 확장성이 좋으나 여전히 사람이 직접 관여해야하는 부분이 많음

<br/>

- **3. 머신러닝/딥러닝 기반**

    - 보다 발전된 데이터 기반 접근방식으로써 머신러닝 기반에서는 특징 추출이 굉장히 중요

    - 머신러닝은 데이터에서 중요한 패턴(특징)을 뽑아내는 게 핵심

    - 딥러닝에 이르러 별도의 특징 추출 중요도가 감소 -> 전에 없던 의미 분석 및 자연어 생성 성능을 확보함
    
    - 딥러닝은 스스로 중요한 특징을 찾아냄-> 의미 파악과 문장 생성 능력이 크게 좋아짐
...
- **1. 언어의 중의성**
    ...
    - 동음이의어
        ...
        - ex : "배"라는 단어는 과일, 탈것, 신체 부위 등 여러 뜻이 있어서 애매함

    - 문맥을 고려해야 함 (예: 반어법)

        - ex : "정말 잘한다"가 비꼬는 말일 수도 있음
...
- **2. 규칙의 예외**
    ...
    - 자연어 문법에는 예외가 많음
        ...
        - 문법 규칙대로라면 맞지 않는데 실제 언어 사용에서는 예외적인 표현이 많아, 모든 경우를 다 다루기가 어려움

<br/>

- **3. 언어의 유연성 및 확장성**

    - 같은 의미를 다양한 방식으로 표현 가능
    
        - 똑같은 뜻을 가진 말을 수십 가지 방법으로 바꿔 말할 수 있다.

    - 단어/구문 추가로 무한히 확장 가능
        
        - 문장은 끝없이 길어질 수 있어서, 규칙만으로는 모든 문장을 처리하기 사실상 불가능하다.
<br/>
...
![System Resources](../../images/Natural%20Language%20Processing%20images/NLP파이프라인.png)
...
- ex 사용자 입력: "오늘 서울 날씨 알려줘"
    ...
    -  **파싱** : 단어 단위로 분리 -> 품사 추
        ...
        - 품사 태깅 : 오늘(Noun), 서울(ProperNoun), 날씨(Noun), 알려줘(Verb)
     
        - NER(개체명 인식) : “오늘” → 시간(time), “서울” → 위치(location) (명사 중심 정보만 파악)

    - **분석** : 의미를 더 깊이 분석 
 
        - 의도(Intent): “알려줘”라는 동사가 질문/요청 의도임을 파악 → intent = 날씨 조회 (여기서 동사로 의도 파악)

        - 개체(Entity): 시간(time) = 오늘, 위치(location) = 서울, 대상(target) = 날씨

        - 문맥적 이해 : 사용자가 지금 “서울의 오늘 날씨가 궁금하다”는 의미적 요청임을 해석
    ...
    - **실행** : 여러 응답 중 하나를 선택
        ...
        - 최종 출력: "오늘 서울의 날씨는 맑음입니다."

<br/>

- NLP 파이프라인 처리층 이미지

![System Resources](../../images/Natural%20Language%20Processing%20images/NLP파이프라인처리층.jpg)
