...
- set (집합) : 순서 x
    ...
    - 부분집합 (Subsets) : A의 모든 원소가 B의 원소라면 A는 B의 부분집합 (A ⊆ B)
    ...
    - 진부분집합(proper subset) : A가 B의 부분집합이지만, A와 B가 같지 않는 경우
 
    - Power Sets (멱집합) : 어떤 집합 A의 모든 부분집합들의 집합
 
        - ex: A = {a,b} 라면 P(A) = {∅, {a}, {b}, {a,b}} 여기서 **공집합도 추가하는게 중요**
   
    - 가산집합 : 셀 수 있는 집합 (ex: 자연수 집합, 정수 집합, 유리수 집합)
 
    - 비가산 : 셀 수 없는 집합 (ex: 실수 집합, 구간 집합 [0,1])
 
        - **핵심은 무한집합이라고 해서 무조건 비가산 집합이 아니다**
     
        - 증명 방법 : 일대일 대응 or 논리적인 수식 
...
- 행렬 (matrix)
    ...
    - 단위행렬(identity matrix) : 대각선만 모두 1인 행렬
 
        -  일반 곱셉식의 1과 같은 역할
     
        -  행렬 A의 0제곱은 단위행렬 In으로 정의
     
    - 전치행렬(Transpose of a Matrix) : 행과 열을 서로 바꿔 얻은 행렬
 
    - 대칭행렬(Symmetric Matrix) : 정방행렬 A가 자신의 전치행렬과 같은 행렬
 
        - 전치(transpose)했을 때 모양이 바뀌지 않으면 대칭행렬 
 
    - 0-1 행렬 : 행렬의 값이 오직 0과 1만 가지는 행렬
